...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title" id="table-title">{{ __('Destination Lists') }}</h3>
        </div>
        <div class="card-body">
            <div class="row">
                        ...
                        <table id="example1" class="table table-bordered table-striped dataTable dtr-inline collapsed"
                            ...
                            <thead>
                                <tr role="row">
                                    ...
                                    <th class="sorting sorting_asc" tabindex="0" aria-controls="example1" rowspan="1"
                                        colspan="1" aria-sort="ascending"
                                        aria-label="Name: activate to sort column descending">Building</th>
                            ...
                            <tbody>
                                @foreach ($destinations as $destination)
                                    <tr>
                                        <td class="dtr-control sorting_1" tabindex="0">{{ $destination->name }}</td>
                                        <td class="dtr-control sorting_1" tabindex="0">{{ $destination->building->name }}
                                        </td>
                                        <td class="dtr-control sorting_1" tabindex="0">
                                            <div class="d-flex justify-content-center">
                                                <a data-toggle="modal" id="modal-update-btn" data-target="#modal-update"
                                                    data-attr="{{ route('admin.destination.edit', $destination) }}"
                                                    class="btn btn-sm btn-warning mr-2">Edit</a>
                                                <form action="{{ route('admin.destination.delete', $destination) }}"
                                                    method="POST"
